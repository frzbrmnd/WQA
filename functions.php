<?php
    ...
    function createNewUser($username, $email, $password){
        global $password_db, $host, $name_db;
        $invite_code = createInviteCode(20);
        $db_connection = pg_connect("host={$host} dbname={$name_db} user=postgres password={$password_db}") or die('connection failed' . pg_last_error());
    ...
    function insertDataIntoTable($tableName, $username, $projectName, $data){
        ...
        if($tableName == "environmental"){
            for($i=0; $i<count($data); $i++){
                ...
                //$query .= "VALUES ({$project_id}, {$data[$i][0]}, {$data[$i][1]}, {$data[$i][2]}, {$data[$i][3]}, {$data[$i][4]});";
                $query .= "VALUES ({$project_id}, {$i}, {$data[$i][0]}, {$data[$i][1]}, {$data[$i][2]}, {$data[$i][3]});";

            }
        }else{
            for($i=0; $i<count($data); $i++){
                $query .= "INSERT INTO {$tableName}(project_id, day_number, temp, po4, nh4, no3, cbod, alga, do_1) ";
                //$query .= "VALUES ({$project_id}, {$data[$i][0]}, {$data[$i][1]}, {$data[$i][2]}, {$data[$i][3]}, {$data[$i][4]}, {$data[$i][5]}, {$data[$i][6]}, {$data[$i][7]});";
                $query .= "VALUES ({$project_id}, {$i}, {$data[$i][0]}, {$data[$i][1]}, {$data[$i][2]}, {$data[$i][3]}, {$data[$i][4]}, {$data[$i][5]}, {$data[$i][6]});";    
            }
        }
        $result = pg_query($query);
        $status = pg_affected_rows($result);
    ...
    function retriveFeatures($username, $projectName){
        global $password_db, $host, $name_db;
        $features = array();$points = array();$lines = array();$polygons = array();$circles = array();
        $project_id = retriveProjectId($username, $projectName);
        $db_connection = pg_connect("host={$host} dbname={$name_db} user=postgres password={$password_db}") or die('connection failed' . pg_last_error());
    ...
    function getInviteLink($username){
        ...
        if(empty($inviteCode)){
            ...
            return "http://localhost:8080/waterQualityAssessment/index.php?guest=" . $inviteCode;
        }
    }  
    function getDataToProcess($username, $projectName){
        ...
        global $password_db, $host, $name_db;
        $db_connection = pg_connect("host={$host} dbname={$name_db} user=postgres password={$password_db}") or die('connection failed' . pg_last_error());
        $query = "SELECT branch_1.temp, branch_1.po4, branch_1.nh4, branch_1.no3, branch_1.cbod, branch_1.alga, branch_1.do_1, branch_2.temp, branch_2.po4, branch_2.nh4, branch_2.no3, branch_2.cbod, branch_2.alga, branch_2.do_1, environmental.t_air, environmental.wind, environmental.q_out, environmental.elevation ";
    ...
    /*function getResults($username, $projectName){
        ...
        global $password_db, $host, $name_db;
        $db_connection = pg_connect("host={$host} dbname={$name_db} user=postgres password={$password_db}") or die('connection failed' . pg_last_error());
        $query = "SELECT result_value from results where project_id={$project_id};";
        $result = pg_query($query);
        while ($row = pg_fetch_row($result)){
            array_push($res, (float)$row[0]);
        }
        pg_free_result($result);
        pg_close($db_connection);
        if(empty($res)){
    ...
    /*function insertResults($username, $projectName, $results){
        $project_id = retriveProjectId($username, $projectName);
        global $password_db, $host, $name_db;
        $db_connection = pg_connect("host={$host} dbname={$name_db} user=postgres password={$password_db}") or die('connection failed' . pg_last_error());
        ...
        for($i=0; $i<count($results); $i++){
            $query .= "INSERT INTO results(project_id, result_value) ";
            $query .= "VALUES ({$project_id}, {$results[$i]});";
        }
        $result = pg_query($query);
        $status = pg_affected_rows($result);
        pg_free_result($result);
